...
assert.equal(result.aboutDialogVisible, true);
assert.equal(result.aboutDialogTitle, "悬浮小浏览器");
assert.match(result.aboutDialogDeclaration, /完全由 OpenAI Codex 生成/);
assert.equal(result.aboutDialogVersion, "1.8.4");
assert.equal(result.aboutDialogCloseLabel, "关闭");
assert.equal(result.aboutDialogClosed, true);
